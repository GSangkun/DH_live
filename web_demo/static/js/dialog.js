//let server_url = "http://47.115.32.142:8888/eb_stream"
let server_url = "http://localhost:8890/eb_stream"
...
// 语音输入逻辑
voiceInputArea.addEventListener('mousedown', async (event) => {
    ...
    if (isVoiceMode && !isRecording) {
        try {
            ...
            mediaRecorder.onstop = () => {
                ...
                isRecording = false;
                voiceInputText.textContent = '按住说话'; // 恢复文字
            };

            mediaRecorder.start();
            isRecording = true;
            voiceInputText.textContent = '松开结束'; // 修改文字
            console.log('开始录音');
...
// 鼠标移出区域时也停止录音
voiceInputArea.addEventListener('mouseleave', () => {
    if (isRecording && mediaRecorder && mediaRecorder.state === 'recording') {
        mediaRecorder.stop();
        console.log('鼠标移出区域，停止录音');
    }
});

// 触摸设备支持
voiceInputArea.addEventListener('touchstart', async (event) => {
    event.preventDefault(); // 阻止默认行为
...
// 触摸结束时停止录音
voiceInputArea.addEventListener('touchend', () => {
    if (isRecording && mediaRecorder && mediaRecorder.state === 'recording') {
        mediaRecorder.stop();
        console.log('停止录音（触摸结束）');
    }
});

// 初始化时设置文本
voiceInputText.textContent = '按住说话';
